...
class ListSpec extends FlatSpec {
  ...
  it should "return a new list with all but the last element again" in {
    assert(init(dropList2)===List(4,3,2))
  }
  ...
  //3.10
  "foldLeft" should "return last element plus 1" in {
    assert(foldLeft(dropList, 0)((_,x) => x + 1) === 5)
  }

  "foldLeft" should "return sum" in {
    assert(foldLeft(dropList, 0)((x,y) => x + y) === 10)
  }

  "foldLeft" should "return product" in {
    assert(foldLeft(dropList, 1)((x,y) => x * y) === 24)
  }
  ...
  it should "return the reversed list for a element list" in {
